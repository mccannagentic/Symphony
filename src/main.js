...
const mkText = (txt, size = 0.1, color = colors.text, width = 3, align = 'left') => {
  ...
  sprite.color = color;
  sprite.anchorX = align;
  sprite.anchorY = 'middle';

  sprite.sync = () => {
    const px = Math.max(20, Math.floor(sprite.fontSize * 220));
    const c = sprite.color?.isColor ? `#${sprite.color.getHexString()}` : (sprite.color || '#d9e4ff');
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.font = `${px}px Inter, Arial, sans-serif`;
    ctx.fillStyle = c;
    ctx.textBaseline = 'middle';
    ctx.textAlign = sprite.anchorX === 'center' ? 'center' : 'left';
    const pad = 18;
    const x = sprite.anchorX === 'center' ? canvas.width * 0.5 : pad;
    const y = canvas.height * 0.52;

    const maxPx = Math.max(120, Math.floor(sprite.maxWidth * 260));
    let text = String(sprite.text || '');
    while (ctx.measureText(text).width > maxPx && text.length > 1) text = text.slice(0, -1);
    if (text !== sprite.text && text.length > 1) text = `${text.slice(0, -1)}…`;

    ctx.fillText(text, x, y);
    texture.needsUpdate = true;

    const w = Math.max(0.12, Math.min(sprite.maxWidth, (ctx.measureText(text).width + pad * 2) / 260));
    const h = Math.max(0.08, sprite.fontSize * 1.9);
